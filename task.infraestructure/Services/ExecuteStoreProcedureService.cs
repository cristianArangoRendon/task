...
{
    public class ExecuteStoreProcedureService(ILogService logService, IDataContext dataContext, ISqlCommandService sqlCommandService) : IExecuteStoreProcedureService
    {
        ...
        public async Task<ResponseDTO> ExecuteTableStoredProcedure<TResult>(string storedProcedureName, object? parameters, Func<SqlDataReader, List<TResult>> mapFunction)
        {
            ...
            try
            {
                ...
                {
                    var firstValue = reader.GetValue(0)?.ToString();
                    if (firstValue == "No Access")
                    {
                        response.Message = "You don't have permissions to perform this operation.";
                        return response;
                    }
                }
